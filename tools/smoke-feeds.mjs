  ...
  ws.on('message',(r)=>{const m=JSON.parse(r);
    ...
    if(m.op==='book'){out.books++;
      {const mid=(m.bids[0][0]+m.asks[0][0])/2;
        ...
        out.res=`mid=${mid.toFixed(4)} lv=${m.levels[0]}/${m.levels[1]} sent=${m.bids.length}/${m.asks.length} span=${span(m.bids)}..${span(m.asks)} notional=${fmt(sum(m.bids))}/${fmt(sum(m.asks))} vol24=${fmt(m.vol24h)} src=${m.source}`;}
    }});
  ws.on('error',e=>{out.err=e.message;clearTimeout(t);res(out)});
});
